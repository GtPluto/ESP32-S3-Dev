...
BikeData::BikeData()
{
    ...
    // 初始化当前状态
    current_speed = 0.0;   // 从0开始逐渐增加
    current_cadence = 0.0; // 从0开始逐渐增加
...
void BikeData::updateSpeed()
{
    ...
    // 计算轮转数
    // 1. 将速度从 km/h 转换为 m/s: speed * (1000/3600)
    // 2. 计算每秒的轮转数: (speed in m/s) / wheel_circumference
    // 3. 计算50ms内的轮转数
...
void BikeData::updatePower()
{
    ...
    // 添加一些随机波动（±5%）
    float rand_factor = 1.0 + (random(-50, 50) / 1000.0);
    power *= rand_factor;
